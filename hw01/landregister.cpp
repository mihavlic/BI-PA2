...
template<typename T>
using EntryOrdering = int (*)(const T&, const T&);
...
class TrashSet {
  ...
  public:
    ...
    template<typename E>
    IndexRange
    find_slice_by_key(const E& key, const E& (*extract)(const T&)) const {
        BinarySearch start = binary_search_by<T>(
            m_data.data(),
            m_data.size(),
            [extract, &key](const T& element) -> int {
...
class CIterator {
  ...
  public:
    ...
    std::string city() const {
        return (*inner.start)->city;
    }

    std::string addr() const {
        return (*inner.start)->addr;
    }

    std::string region() const {
        return (*inner.start)->region;
    }

    unsigned id() const {
        return (*inner.start)->id;
    }

    std::string owner() const {
        return (*inner.start)->owner.inner;
    }
...
int cmp_entry_city_addr(const GoodParsingRules& a, const GoodParsingRules& b) {
    return cmp_two<std::string, std::string>(
        a->city,
        a->addr,
        b->city,
        b->addr
...
int cmp_entry_owner_counter(
    const GoodParsingRules& a,
    const GoodParsingRules& b
) {
    return cmp_two<CaseInsensitive, unsigned>(
        a->owner,
        a->counter,
        b->owner,
        b->counter
...
class CLandRegister {
  ...
  public:
    ...
    bool
    getOwner(const std::string& region, unsigned int id, std::string& owner)
        const {
        LandEntry tmp {
            "",
            0,
    ...
    CIterator listByOwner(const std::string& owner) const {
        ...
        IndexRange range = owner_counter.find_slice_by_key<CaseInsensitive>(
            a,
            [](LandEntry* const& t) -> const CaseInsensitive& {
                return t->owner;
            }
        );

        return CIterator(
            owner_counter.begin() + range.start,
...
static void test0() {
    ...
    assert(
        !i5.atEnd() && i5.city() == "Prague" && i5.addr() == "Thakurova"
        && i5.region() == "Dejvice" && i5.id() == 12345 && i5.owner() == "CVUT"
    );
    i5.next();
    assert(
        !i5.atEnd() && i5.city() == "Prague" && i5.addr() == "Technicka"
        && i5.region() == "Dejvice" && i5.id() == 9873 && i5.owner() == "CVUT"
    );
    i5.next();
    assert(
        !i5.atEnd() && i5.city() == "Liberec" && i5.addr() == "Evropska"
        && i5.region() == "Librec" && i5.id() == 4552 && i5.owner() == "Cvut"
    );
    i5.next();
    assert(
        !i5.atEnd() && i5.city() == "Plzen" && i5.addr() == "Evropska"
    ...
    assert(
        !i6.atEnd() && i6.city() == "Prague" && i6.addr() == "Thakurova"
        && i6.region() == "Dejvice" && i6.id() == 12345 && i6.owner() == "CVUT"
    );
    i6.next();
    assert(
        !i6.atEnd() && i6.city() == "Prague" && i6.addr() == "Technicka"
...
int main(void) {
    ...
    int a = 5;
    IndexRange range =
        set.find_slice_by_key<int>(a, [](const IntTuple& t) -> const int& {
            return t.x;
        });
