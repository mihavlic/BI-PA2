...
template <typename T> using KeySearch = int(T *);
template <typename T> using KeySearchPredicate = bool(T *);
...
template <typename T> struct Node {
  ...
  */
  ...
  static void find_min_by(Node<T> **node, KeySearchPredicate<T> search,
  ...
  static void find_max_by(Node<T> **node, KeySearchPredicate<T> search,
                          std::vector<Node<T> **> &path) {
    while (true) {
      Node<T> *root = *node;
  ...
  static void find(Node<T> **node, KeySearch<T> search,
                   std::vector<Node<T> **> &path) {
    while (true) {
      path.push_back(node);
...
public:
  ...
  void insert(T key) { Node<T>::insert(&this->root, key, this->compare); }
  void remove(T key) { Node<T>::remove(&this->root, key, this->compare); }
